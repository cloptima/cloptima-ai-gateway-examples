...
boundary this queue exists to enforce, so this example stops at "request it,
show it's pending" and leaves the decision to a second privileged identity
in the console's Audit tab, same as any real governance workflow would
require.
...
def main():
    ...
    print("  Note: approvalType 'budget_limit_change' requires an 'owner'-role identity to REVIEW/decide - requesting it just needs owner-or-admin, so this ai:admin key can request it fine.")
    ...
    print(
        ...
        f"${CURRENT_DAILY_BUDGET_USD} until a second, owner-role identity reviews and approves it via "
        "reviewLLMGatewayApproval - not something this script does on its own behalf."
    )
    print(f"Evidence: Audit tab ({config.CONSOLE['audit']}) - shows the pending budget_limit_change request awaiting review; Policies tab ({config.CONSOLE['policies']}) shows the policy's current, unchanged budget.")
