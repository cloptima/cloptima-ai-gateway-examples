...
// enforce mode needs an additional per-app/route/model-family approval on top
// of the policy flag; see semantic-cache-enforce.mjs for that full flow).
// Repeats one exact prompt for exact-cache evidence, then sends paraphrased
// variants for semantic-cache evidence. There is no client-side cache toggle -
// this is entirely policy-driven server-side; see ../../docs/CACHE_AND_POLICY.md.
// Run standalone:
