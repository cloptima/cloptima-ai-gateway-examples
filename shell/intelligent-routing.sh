...
# demonstrated here.
# Run standalone: ./intelligent-routing.sh
set -euo pipefail
...
SUFFIX="$(run_suffix)"
APP_ID="intelligent-routing-$SUFFIX"

echo "Creating a policy with intelligent routing in observe mode across cheap/balanced/strong candidate tiers..."
POLICY=$(create_policy "$(jq -n \
  --arg name "intelligent-routing-$SUFFIX" \
...
KEY=$(create_virtual_key "$(jq -n --arg name "vk-intelligent-routing-$SUFFIX" --arg appId "$APP_ID" \
...
echo "Evidence: Audit tab ($CONSOLE_AUDIT) - shows the logged routing-decision trail per call; Policies tab ($CONSOLE_POLICIES) shows the metadata.routing.intelligent config, including the candidate tiers above."
