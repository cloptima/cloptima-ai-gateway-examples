...
| Audit | `/llm/audit` | Blocked-request records, pending governance-queue approvals, and routing-decision trails - policy, provider, model, token, guardrail, and agentic-limit blocks, each with the reason and violation detail. | `rate-limit`, `token-limit`, `budget-limit`, `agentic-runaway`, `pii-guardrail`, `provider-deny`, `metadata-deny`, `approval-workflow`, `guardrail-cost-governance`, `guardrail-detector-categories`, `intelligent-routing`, `mcp-tool-governance`, `prompt-release-workflow`, `semantic-cache-enforce` |
